...
conditions = [
    ("Tukey", "Points On", "Jitter On"),
    ("MinMax", "Points On", "Jitter On"),
    ("Tukey", "Points On", "Jitter Off"),
    ("MinMax", "Points On", "Jitter Off"),
    ("Tukey", "Points Off"),
    ("MinMax", "Points Off")
]


def unpack_condition(condition):
    """Return (whisker, points, jitter_or_None) for a condition tuple."""
    whisker = condition[0]
    points = condition[1]
    jitter = condition[2] if len(condition) > 2 else None
    return whisker, points, jitter
...
def plot_trial(left, right, condition, trial_type, trial_idx):
    ...
    condition_parts = [whisker, points]
    if points == "Points On" and jitter:
        condition_parts.append(jitter)
    plt.title(f"{' x '.join(condition_parts)}")
    
    # Determine which design folder this plot belongs to
    design_folder = get_design_folder(condition)
    
    condition_slug_parts = [
        whisker.replace(' ', ''),
        points.replace(' ', '')
    ]
    if points == "Points On" and jitter:
        condition_slug_parts.append(jitter.replace(' ', ''))
    condition_slug = "_".join(condition_slug_parts)
    filename = f"{trial_idx}_{trial_type.replace(' ', '_')}_{condition_slug}.png"
...
with open(csv_path, "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["TrialIdx", "TrialType", "Instance", "Whisker", "Points", "Jitter", "Left_SD", "Right_SD", "More_Variable"])
    ...
    for trial_type in trial_types:
        for instance in range(num_instances_per_trial_type):
            ...
            for condition in conditions:
                ...
                # log one row
                writer.writerow([
                    trial_idx, trial_type, instance, whisker, points, jitter or "",
